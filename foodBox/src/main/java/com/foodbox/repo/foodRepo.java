...
public interface foodRepo extends JpaRepository<food, Integer>{
	...
	@Query("SELECT f FROM food f WHERE f.status = 'yes'")
	...
	@Query("SELECT f FROM food f WHERE f.status LIKE 'yes' AND (f.category LIKE 'veg' OR f.category LIKE 'nonveg') order by f.price")
